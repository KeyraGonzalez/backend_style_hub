...
export class StripeService {
  ...
  async createCheckoutSession(sessionData: {
    ...
    try {
      ...
      // Construir los datos de la sesión de checkout manualmente para evitar problemas con el flattenObject
      ...
      // Line items - construir manualmente el array
      formData.append(
      ...
      this.logger.log(
        `Creando Checkout Session para pedido ${orderId} con monto $${amount} ${currency}`
      );
      ...
      this.logger.log(
        `Checkout Session Stripe creada exitosamente: ${session.id} para pedido ${orderId}`
      );
      ...
      this.logger.error(
        `Error al crear Checkout Session Stripe para pedido ${sessionData.orderId}:`,
        error.response?.data || error.message
      );
