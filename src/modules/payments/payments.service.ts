...
export class PaymentsService {
  ...
  async createStripeCheckoutSession(
    ...
    try {
      const sessionData = {
        ...
        customerEmail: user?.email,
        successUrl: `${process.env.FRONTEND_URL}/checkout/success?session_id={CHECKOUT_SESSION_ID}&order_id=${orderId}`,
        cancelUrl: `${process.env.FRONTEND_URL}/checkout/cancel?order_id=${orderId}`,
        metadata: {
          order_id: orderId,
          user_id: userId,
        },
      ...
      if (!result.success) {
        throw new BadRequestException(
          result.error || 'Error creando sesión de checkout'
        );
      }
      ...
      this.logger.error('Error creando sesión de checkout Stripe:', error);
      throw new BadRequestException(
        `Error creando sesión de checkout: ${error.message}`
      );
    }
  }
   ...
   */
  async createPayPalOrder(orderId: string, userId: string): Promise<any> {
    // Validar que la orden existe y pertenece al usuario
    const order = await this.orderModel.findOne({ _id: orderId, userId });
    if (!order) {
      throw new NotFoundException('Orden no encontrada');
    ...
    try {
      ...
      if (!result.success) {
        throw new BadRequestException(
          result.error || 'Error creando orden de PayPal'
        );
      }
      ...
      throw new BadRequestException(
        `Error creando orden PayPal: ${error.message}`
      );
    }
  }
